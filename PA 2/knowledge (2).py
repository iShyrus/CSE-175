...
monster_sentences = [
    ...
    'Person(Bob)',
    'Person(Lin)',
    'Person(Maurice)',# Bob, Lin, and Maurice are people.

    'Person(x) & Dead(x) ==> Victim(x)',# A person who is dead is a victim.

    'Vampire(x)==> Monster(x)',
    'Werewolf(x)==> Monster(x)',
    'Witch(x)==> Monster(x)',# All vampires, werewolves, and witches are monsters.

    'Monster(x) & Present(x) ==> Suspect(x)',# Any monster present at the crime scene is a suspect.

    '(Suspect(x) & Vampire(x)) & (Victim(y) & Bitten(y)) ==> Killed(x,y)',    # If a vampire is suspected and the victim was bitten, then
    #   the vampire killed the victim.

    '(Suspect(x) & Werewolf(x)) & (Victim(y) & Eaten(y)) ==> Killed(x,y)',    # If a werewolf is suspected and the victim was eaten, then
    #   the werewolf killed the victim.

    '(Suspect(x) & Witch(x)) & (Victim(y) & Poisoned(y)) ==> Killed(x,y)',    # If a witch is suspected and the victim was poisoned, then
    #   the witch killed the victim.

    'Drained(x) & Intact(x) ==> Bitten(x)',    # If a victim is drained of blood but has an intact body, then
    #   the victim was bitten.

    'Drained(x) & Incomplete(x) ==> Eaten(x)',    # If a victim is drained of blood and has body pieces missing (i.e., is
    #   incomplete), then the victim was eaten.

    'Intact(x) & Complexion(x,Blue) ==> Poisoned(x)',
    'Intact(x) & Complexion(x,Green) ==> Poisoned(x)',
    'Intact(x) & Complexion(x,Purple) ==> Poisoned(x)',
    'Complexion(x,Pale) & Boils(x) ==> Poisoned(x)',    # If a victim's body is intact, and the victim's skin complexion is
    #   green, blue, or purple, or if it is pale with boils, then
    #   the victim was poisoned.

    'Complexion(x,Pale) & Cold(x) ==> Drained(x)',
    'Complexion(x,Pale) & Punctured(x) ==> Drained(x)',    # If a victim's skin complexion is pale and the body is either
    #   cold or punctured, then the victim is drained of blood.

    'Dismembered(x) ==> Incomplete(x)',
    'Disemboweled(x) ==> Incomplete(x)',    # If a victim's body is complete (i.e., not incomplete), then
    #   it is not dismembered or disemboweled.
]
