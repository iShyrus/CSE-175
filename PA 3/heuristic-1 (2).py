...
#
# Cyrus Adriel Arellano 12/1/2022
...
def expected_value_over_delays(state, ply):
    ...
    val = 0.0
    #For loop to go through time steps
    for x in range(min_time_steps, max_time_steps + 1):
        # Gets value of state
        stateValue = value(state, ply)

        # Gets probability of time
        state.time_remaining = x
        probOfTime = probability_of_time(x)

        #Multiply state value and time and add to value
        val += stateValue * probOfTime
...
def heuristic_value(state):
    ...
    hval = 0
    #Subtract West Location to East Location
    sub = abs(state.w_loc) - abs(state.e_loc)

    #Chooses best option if computer losing to help win
    if  abs(state.e_loc) < abs(state.w_loc):
        hval = max_payoff / (sub + 5)
    
    #Chooses best option if computer not currently losing
    elif abs(state.e_loc) >= abs(state.w_loc):
        hval = -max_payoff

    if Player.east == state.current_turn:
        hval = -hval
